...
import { RsiResponse } from './types';

const provider = RapidApiProvider.CryptoRSI;
const baseUrl = 'https://crypto-indicators-rest.p.rapidapi.com';

function createCryptoRsiApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Crypto RSI client', { provider, kind: 'network_error' }));
    },
  };
}

export const cryptoRsiClient = createCryptoRsiApiClient();

export async function getRsi(symbol: string, interval: string): Promise<RsiResponse | RapidApiError> {
  const result = await cryptoRsiClient.get<RsiResponse>(`/rsi/${symbol}/${interval}`);
  return result;
}
