...
import { RealTimeFinanceResponse } from './types';

const provider = RapidApiProvider.RealTimeFinance;
const baseUrl = 'https://apidojo-yahoo-finance-v1.p.rapidapi.com';

function createRealTimeFinanceApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Real-time Finance client', { provider, kind: 'network_error' }));
    },
  };
}

export const realTimeFinanceClient = createRealTimeFinanceApiClient();

export async function getQuotes(symbols: string[]): Promise<RealTimeFinanceResponse | RapidApiError> {
  const result = await realTimeFinanceClient.get<RealTimeFinanceResponse>('/market/v2/get-quotes', { symbols: symbols.join(',') });
  return result;
}
