...
import { TurkeyFinancialResponse } from './types';

const provider = RapidApiProvider.TurkeyFinancial;
const baseUrl = 'https://bist100-stock-data-15-minutes-late.p.rapidapi.com';

function createTurkeyFinancialApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Turkey Financial client', { provider, kind: 'network_error' }));
    },
  };
}

export const turkeyFinancialClient = createTurkeyFinancialApiClient();

export async function getBist100(): Promise<TurkeyFinancialResponse | RapidApiError> {
  const result = await turkeyFinancialClient.get<TurkeyFinancialResponse>('/');
  return result;
}
