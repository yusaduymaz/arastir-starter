...
import { CryptoNewsResponse } from './types';

const provider = RapidApiProvider.CryptoNews;
const baseUrl = 'https://crypto-news-api1.p.rapidapi.com';

function createCryptoNewsApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Crypto News client', { provider, kind: 'network_error' }));
    },
  };
}

export const cryptoNewsClient = createCryptoNewsApiClient();

export async function getNews(source?: string): Promise<CryptoNewsResponse | RapidApiError> {
  const path = source ? `/news/${source}` : '/news';
  const result = await cryptoNewsClient.get<CryptoNewsResponse>(path);
  return result;
}
