...
import { YahooRealTimeResponse } from './types';

const provider = RapidApiProvider.YahooRealTime;
const baseUrl = 'https://apidojo-yahoo-finance-v1.p.rapidapi.com';

function createYahooRealTimeApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Yahoo Real Time client', { provider, kind: 'network_error' }));
    },
  };
}

export const yahooRealTimeClient = createYahooRealTimeApiClient();

export async function getQuotes(symbols: string[]): Promise<YahooRealTimeResponse | RapidApiError> {
  const result = await yahooRealTimeClient.get<YahooRealTimeResponse>('/market/v2/get-quotes', { symbols: symbols.join(',') });
  return result;
}
