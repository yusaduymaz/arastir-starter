...
import { YHFinanceResponse } from './types';

const provider = RapidApiProvider.YFinance;
const baseUrl = 'https://apidojo-yahoo-finance-v1.p.rapidapi.com';

function createYHFinanceApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for YH Finance client', { provider, kind: 'network_error' }));
    },
  };
}

export const yhFinanceClient = createYHFinanceApiClient();

export async function getQuotes(symbols: string[]): Promise<YHFinanceResponse | RapidApiError> {
  const result = await yhFinanceClient.get<YHFinanceResponse>('/market/v2/get-quotes', { symbols: symbols.join(',') });
  return result;
}
