...
import { HaremAltinResponse } from './types';

const provider = RapidApiProvider.HaremAltin;
const baseUrl = 'https://harem-altin-live-gold-price-data.p.rapidapi.com';

function createHaremAltinApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Harem Altin client', { provider, kind: 'network_error' }));
    },
  };
}

export const haremAltinClient = createHaremAltinApiClient();

export async function getGoldPrices(): Promise<HaremAltinResponse | RapidApiError> {
  const result = await haremAltinClient.get<HaremAltinResponse>('/');
  return result;
}
