...
const provider = RapidApiProvider.ExchangeRates;
const baseUrl = 'https://exchange-rate-api1.p.rapidapi.com';

function createExchangeRatesApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Exchange Rates client', { provider, kind: 'network_error' }));
    },
  };
}

export const exchangeRatesClient = createExchangeRatesApiClient();

export async function getLatestRates(baseCurrency: string): Promise<ExchangeRatesResponse | RapidApiError> {
  const result = await exchangeRatesClient.get<ExchangeRatesResponse>(`/latest/${baseCurrency}`);
  return result;
}
