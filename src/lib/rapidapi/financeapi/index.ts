...
import { YahooFinanceResponse } from './types';

const provider = RapidApiProvider.FinanceAPI;
const baseUrl = 'https://apidojo-yahoo-finance-v1.p.rapidapi.com';

function createFinanceApiClient(): RapidApiClient {
  ...
  return {
    ...
    post: async <T>(): Promise<T | RapidApiError> => {
      ...
      return Promise.resolve(new RapidApiError('POST method not supported for Finance API client', { provider, kind: 'network_error' }));
    },
  };
}

export const financeApiClient = createFinanceApiClient();

export async function getQuotes(symbols: string[]): Promise<YahooFinanceResponse | RapidApiError> {
  const result = await financeApiClient.get<YahooFinanceResponse>('/market/v2/get-quotes', { symbols: symbols.join(',') });
  return result;
}
